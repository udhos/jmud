...
class Attack extends Command {
	...
	if (aChar.isFighting()) {
	    aChar.send("Você já está lutando!");
	    return;
	}

	if (!toker.hasMoreTokens()) {
	    aChar.send("Quem você quer atacar?");
	    return;
	}
	...
	if (vict == null) {
	    aChar.send("Não há ninguém com esse nome aqui.");
	    return;
	}

	if (vict == aChar) {
	    aChar.send("Uma idéia um tanto quanto suicida, não?");
	    return;
	}
