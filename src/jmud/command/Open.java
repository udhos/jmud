...
class Open extends Command {
    ...
    Open(String name, int mRank, int mPos, int opt) {
	...
	if (!toker.hasMoreTokens()) {
	    aChar.send("O que você quer abrir?");
	    return;
	}
	...
	if (DOOR.startsWith(trg)) {
	    ...
	    if (!toker.hasMoreTokens()) {
		aChar.send("Qual porta você quer abrir?");
		return;
	    }
	    ...
	    switch(dr.getStatus()) {
	    case Door.S_OPEN:
		aChar.send("A porta já está aberta.");
		break;
	    case Door.S_CLOSED:
		dr.setBothStatus(Door.S_OPEN, rm);
		plc.action("$p abre a porta " + dr.getDirectionName() + ".", false, aChar);
		break;
	    case Door.S_LOCKED:
