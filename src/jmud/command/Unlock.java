...
class Unlock extends Command {
    ...
    Unlock(String name, int mRank, int mPos, int opt) {
	super(name, mRank, mPos, opt);
    }

    static final String DOOR = "porta";

    private static boolean canUnlock(Item it, Room rm, Door dr) {
	...
	if (!toker.hasMoreTokens()) {
	    aChar.send("O que você quer destrancar?");
	    return;
	}
	...
	if (DOOR.startsWith(trg)) {
	    ...
	    if (!toker.hasMoreTokens()) {
		aChar.send("Qual porta você quer destrancar?");
		return;
	    }
	    ...
	    if (!dr.isLockable()) {
		aChar.send("Essa porta não pode ser destrancada.");
		return;
	    }
		...
		break;
	    case Door.S_CLOSED:
		aChar.send("A porta já está destrancada.");
		break;
	    case Door.S_LOCKED:
		if (!canUnlock(aChar.getEq(Item.W_LHOLD), rm, dr) && !canUnlock(aChar.getEq(Item.W_RHOLD), rm, dr)) {
		    aChar.send("Você não está segurando a chave adequada.");
		    return;
		}
		dr.setBothStatus(Door.S_CLOSED, rm);
		plc.action("$p destranca a porta " + dr.getDirectionName() + ".", true, aChar);
		break;
	    default:
	    }
