...
class Disconnect extends Command {
    ...
    Disconnect(String name, int mRank, int mPos, int opt) {
	super(name, mRank, mPos, opt);
    }

    static final String SYNTAX = "Sintaxe: desconecte <nome>";
	...
	Char vit = cli.getChar();

	if (aChar.forbidAccess(vit))
	    return;

	Log.info(aChar.getName() + " desconectando " + vit.getName());
	cli.disconnect();
    }
}
